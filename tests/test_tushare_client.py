...
def test_tushare_client_sanitizes_transport_errors_and_repr():
    fake_token = "fake-token-for-tests-client-secret"
...
def test_tushare_client_preserves_sanitized_permission_and_rate_limit_codes():
    secret = "fake-token-for-tests-code-path"
    client = TushareClient(
        transport={
    ...
    assert secret not in str(permission_info.value)
    assert secret not in str(rate_info.value)
