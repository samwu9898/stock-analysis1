...
Implementation status: P1.1 AI Datacenter pilot, CXO expansion, Satellite expansion, Low Altitude expansion, Resource first implementation, Semiconductor first implementation, Advanced Manufacturing first implementation, and Stable Growth first implementation have been implemented and accepted after this v1.1 design. Current P1.1 support is limited to `ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing`, `semiconductor_cycle`, `advanced_manufacturing_growth`, and `stable_growth`; it outputs independent `research_intelligence_p1_<code>.json`, `research_questions_p1_<code>.json`, and `research_questions_p1_<code>.md` artifacts. `output/research_intelligence_p1_*` and `output/research_questions_p1_*` are generated runtime artifacts and should not be committed. AI Datacenter acceptance covered `002837` and `300442`; Satellite primary acceptance covered `601698`; Low Altitude primary acceptance covered `000099`; Resource primary acceptance covered `000426`; Semiconductor primary acceptance covered `002371`; Advanced Manufacturing primary acceptance covered `002050`; Stable Growth primary acceptance covered `600406`. `resource_core` remains design-only, `601899 / 603993` remain later Resource validation / boundary samples, `601689` remains a later Advanced Manufacturing validation / boundary sample, `002028` is Stable Growth validation / boundary only, and `600276` is excluded from Stable Growth first-version support. Semiconductor multi-sample observation has completed: `688012 / 688981 / 603501 / 300604` were validation samples refreshed through upstream raw / fundamental / evidence-pack and P1.1, `300308 / 300476` were boundary / negative samples, and no validation sample was incorrectly mapped to the `002371` equipment first-version logic. Advanced Manufacturing multi-sample observation is complete, and the `601689` boundary data completion smoke is complete. Advanced Manufacturing first-version support is limited to `advanced_manufacturing_growth + 002050`; `601689` was classified as `advanced_manufacturing_growth` but correctly stayed at `unsupported_pilot_strategy` / `not_assessable`, preserved fallback `company_transmission_path`, source-bucket independence, and the safety boundary, and was not force-fit into the `002050` three-business-layer matrix or Tuopu large-customer / robotics revenue realization narrative. Stable Growth first-version support is limited to `stable_growth + 600406`; Stable Growth acceptance recorded `missing / not_assessable = 19/33 = 57.58%`, which is expected and correct. Data-side caveat: `601689` `news` / `latest_news` still reports `Invalid regular expression: invalid escape sequence: \u`; this is an upstream/news issue and does not affect the P1.1 boundary smoke conclusion. Robotics theme exposure remains fallback + `not_assessable` unless independent non-zero robotics revenue, order, customer, mass-production, delivery, or collection evidence exists. Latest recorded pytest result is `484 passed`, and latest recorded regression suite result is `passed=47 failed=0 total=47`.
...
- Do not treat robotics theme, strategic layout wording, or news as revenue realization.
- Do not use refrigeration / air-conditioning or automotive thermal-management evidence as a proxy for robotics realization.
- Do not treat design-win, qualification, or nomination as batch revenue.
- Do not treat receivable growth as high-quality revenue.
- Do not treat inventory movement as demand judgment.
- Do not treat valuation metrics as target price, valuation-high/low, or trading judgment.
...
| `stable_growth` | company-level order / repeat-demand evidence, cash conversion, receivables / collection quality, OCF, ROE / ROIC source, capex discipline, margin stability; grid / infrastructure / SOE attributes are not demand-durability evidence |
...
- P1.1 first implementation was narrowed to the `ai_datacenter_infrastructure` pilot covering `cooling_liquid_cooling_infrastructure` and `datacenter_operator`, before later accepted CXO, Satellite, Low Altitude, Resource (`resource_swing + 000426` only), Semiconductor (`semiconductor_cycle + 002371` only), Advanced Manufacturing (`advanced_manufacturing_growth + 002050` only), and Stable Growth (`stable_growth + 600406` only) expansions;
- Semiconductor first-version scope remains equipment sub-chain first, with materials / fabless / foundry / OSAT handled only as `not_applicable` / `not_assessable` boundaries until a later design / implementation / acceptance cycle expands them;
...
P1.1 implementation status: implemented / accepted for the AI Datacenter pilot, CXO expansion, Satellite expansion, Low Altitude expansion, Resource first implementation, Semiconductor first implementation, Advanced Manufacturing first implementation, and Stable Growth first implementation. The accepted baseline includes `ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing` for primary sample `000426`, `semiconductor_cycle` for primary sample `002371`, `advanced_manufacturing_growth` for primary sample `002050`, and `stable_growth` for primary sample `600406`, with `601698` as the Satellite primary acceptance sample and `000099` as the Low Altitude primary acceptance sample. Resource P1.1 does not support `resource_core`; `601899 / 603993` remain later validation / boundary samples. Semiconductor P1.1 first version is limited to `semiconductor_cycle + 002371` and uses the equipment sub-chain as the first-version path. Advanced Manufacturing P1.1 first version is limited to `advanced_manufacturing_growth + 002050`; `601689` remains a later validation / boundary sample only. Stable Growth P1.1 first version is limited to `stable_growth + 600406`; `002028` is validation / boundary only and `600276` is excluded. The Semiconductor multi-sample observation is complete: `688012 / 688981 / 603501 / 300604` were upstream-refreshed validation samples, `300308 / 300476` were boundary / negative samples, and no validation sample was force-fit into the `002371` equipment first-version logic. The Advanced Manufacturing multi-sample observation is complete, and the `601689` boundary data completion smoke is complete: `601689` was classified as `advanced_manufacturing_growth` but correctly returned `unsupported_pilot_strategy` / `not_assessable`, preserved fallback `company_transmission_path`, source-bucket independence, and the safety boundary, and was not force-fit into the `002050` refrigeration / air-conditioning, automotive thermal-management, and robotics / actuator / emerging-business matrix. Tuopu large-customer and robotics narratives were not written as realized revenue. Stable Growth acceptance recorded `missing / not_assessable = 19/33 = 57.58%`, which is expected and correct. Data-side caveat: `601689` `news` / `latest_news` still reports `Invalid regular expression: invalid escape sequence: \u`; this is an upstream/news issue and does not affect the P1.1 boundary smoke conclusion. Materials / fabless / foundry / OSAT remain `not_applicable` / `not_assessable` boundaries rather than full implementations. Resource guardrails are: commodity price is not company revenue, commodity cycle is not company performance, reserves are not production, production is not sales unless both are disclosed and reconciled, capex is not capacity release, inventory movement is not demand judgment, missing hedging disclosure is neither hedged nor unhedged, and `resource_core` steadiness / dividend capacity must not be written as fact. Semiconductor guardrails are: semiconductor cycle is not company performance; localization narrative is not revenue realization; R&D ratio is not a technology barrier or moat; customer introduction / certification / qualification is not batch revenue; contract liabilities are not backlog; capex is not capacity release, utilization, delivery, or revenue conversion; inventory movement is not demand judgment; export controls / sanctions are not company operating benefit or damage facts without company-level impact evidence. Advanced Manufacturing guardrails are: robotics theme is not revenue realization; strategic layout wording and `news` / `latest_news` are not valid transmission nodes; refrigeration / air-conditioning and automotive thermal-management financial data cannot proxy robotics realization; design-win / qualification / nomination is not batch revenue; contract liabilities are not backlog; capex is not capacity release, mass production, utilization, or revenue conversion; R&D ratio is not a technology barrier; receivable growth is not high-quality revenue; inventory movement is not demand judgment; valuation metrics are evidence-sufficiency context only and must not become target price, valuation-high/low, or trading judgment. Stable Growth guardrails are: the `stable_growth` label is not operating-steadiness evidence; industry rigid demand, infrastructure, policy protection, and SOE / central-SOE attributes are not demand-durability evidence; revenue growth is not stable growth; single-period `revenue_yoy` plus same-direction operating cash flow is not multi-period stable-growth quality; contract liabilities are not backlog; receivable growth is not high-quality revenue; single-period OCF improvement is not long-term cash-flow stability; dividend / payout is not sustainable shareholder return without free cash flow, debt, capex, and earnings-quality support; single-period ROE is not long-term competitiveness; capex is not capacity release, utilization, future revenue, or growth conversion; valuation metrics are evidence-sufficiency context only and must not become valuation-high/low, target price, upside, or trading judgment. The implementation includes `company_transmission_path` schema + builder enforcement, source-bucket independent counting, conservative `not_assessable` handling, and independent P1 JSON / Markdown artifacts. It remains outside connectors, HTML display, Dashboard display, project database, peer benchmark, all-strategy expansion, and live macro / policy data. Latest recorded validation after Stable Growth acceptance: pytest `484 passed`; regression suite `passed=47 failed=0 total=47`.
...
After P1.1 Resource acceptance, keep the Resource baseline frozen at `resource_swing + 000426` unless a new design / implementation / acceptance cycle expands it. After P1.1 Semiconductor acceptance, keep the Semiconductor baseline frozen at `semiconductor_cycle + 002371` unless a new design / implementation / acceptance cycle expands it. After P1.1 Advanced Manufacturing acceptance, keep the baseline frozen at `advanced_manufacturing_growth + 002050`; after P1.1 Stable Growth acceptance, keep the baseline frozen at `stable_growth + 600406`. Do not move directly into P1.2 / P1.3 before the current P1.1 expansion behavior is observed across more samples.
