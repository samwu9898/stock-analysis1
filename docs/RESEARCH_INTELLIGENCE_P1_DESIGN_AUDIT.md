...
- First version implements one pilot `strategy_type` only: `ai_datacenter_infrastructure`.
- The pilot covers two `sub_type` values only:
  - `cooling_liquid_cooling_infrastructure`;
  - `datacenter_operator`.
- Do not implement all `strategy_type` values in the first P1.1 version.
...
- pilot coverage is limited to `ai_datacenter_infrastructure` / `cooling_liquid_cooling_infrastructure` / `datacenter_operator`;
...
- P1.1 first implementation is narrowed to the `ai_datacenter_infrastructure` pilot covering `cooling_liquid_cooling_infrastructure` and `datacenter_operator`;
