...
Implementation status: P1.1 AI Datacenter pilot, CXO expansion, Satellite expansion, Low Altitude expansion, Resource first implementation, and Semiconductor first implementation have been implemented and accepted after this v1.1 design. Current P1.1 support is limited to `ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing`, and `semiconductor_cycle`; it outputs independent `research_intelligence_p1_<code>.json`, `research_questions_p1_<code>.json`, and `research_questions_p1_<code>.md` artifacts. AI Datacenter acceptance covered `002837` and `300442`; Satellite primary acceptance covered `601698`; Low Altitude primary acceptance covered `000099`; Resource primary acceptance covered `000426`; Semiconductor primary acceptance covered `002371`. `resource_core` remains design-only, and `601899 / 603993` remain later validation / boundary samples. Semiconductor multi-sample observation has completed: `688012 / 688981 / 603501 / 300604` were validation samples refreshed through upstream raw / fundamental / evidence-pack and P1.1, `300308 / 300476` were boundary / negative samples, and no validation sample was incorrectly mapped to the `002371` equipment first-version logic. Semiconductor observation can be formally closed. Data-side caveat: validation samples still had `latest_news` missing because the raw news block reported `Invalid regular expression: invalid escape sequence: \u`; this is an upstream/news data issue and does not affect the P1.1 boundary smoke conclusion. Latest recorded pytest result is `464 passed`, and latest recorded regression suite result is `passed=47 failed=0 total=47`.
...
- P1.1 first implementation was narrowed to the `ai_datacenter_infrastructure` pilot covering `cooling_liquid_cooling_infrastructure` and `datacenter_operator`, before later accepted CXO, Satellite, Low Altitude, Resource (`resource_swing + 000426` only), and Semiconductor (`semiconductor_cycle + 002371` only) expansions;
...
P1.1 implementation status: implemented / accepted for the AI Datacenter pilot, CXO expansion, Satellite expansion, Low Altitude expansion, Resource first implementation, and Semiconductor first implementation. The accepted baseline includes `ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing` for primary sample `000426`, and `semiconductor_cycle` for primary sample `002371`, with `601698` as the Satellite primary acceptance sample and `000099` as the Low Altitude primary acceptance sample. Resource P1.1 does not support `resource_core`; `601899 / 603993` remain later validation / boundary samples. Semiconductor P1.1 first version is limited to `semiconductor_cycle + 002371` and uses the equipment sub-chain as the first-version path. The Semiconductor multi-sample observation is complete: `688012 / 688981 / 603501 / 300604` were upstream-refreshed validation samples, `300308 / 300476` were boundary / negative samples, and no validation sample was force-fit into the `002371` equipment first-version logic. Materials / fabless / foundry / OSAT remain `not_applicable` / `not_assessable` boundaries rather than full implementations. Resource guardrails are: commodity price is not company revenue, commodity cycle is not company performance, reserves are not production, production is not sales unless both are disclosed and reconciled, capex is not capacity release, inventory movement is not demand judgment, missing hedging disclosure is neither hedged nor unhedged, and `resource_core` steadiness / dividend capacity must not be written as fact. Semiconductor guardrails are: semiconductor cycle is not company performance; localization narrative is not revenue realization; R&D ratio is not a technology barrier or moat; customer introduction / certification / qualification is not batch revenue; contract liabilities are not backlog; capex is not capacity release, utilization, delivery, or revenue conversion; inventory movement is not demand judgment; export controls / sanctions are not company operating benefit or damage facts without company-level impact evidence. The implementation includes `company_transmission_path` schema + builder enforcement, source-bucket independent counting, conservative `not_assessable` handling, and independent P1 JSON / Markdown artifacts. It remains outside connectors, HTML display, Dashboard display, project database, peer benchmark, all-strategy expansion, and live macro / policy data. Data-side caveat: validation samples still had `latest_news` missing because the raw news block reported `Invalid regular expression: invalid escape sequence: \u`. Latest recorded validation after Semiconductor acceptance: pytest `464 passed`; regression suite `passed=47 failed=0 total=47`.
