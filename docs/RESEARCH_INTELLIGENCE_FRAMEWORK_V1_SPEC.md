...
Research Intelligence P1.1 has an accepted implementation baseline covering the AI Datacenter pilot, CXO expansion, Satellite expansion, Low Altitude expansion, Resource first implementation, Semiconductor first implementation, Advanced Manufacturing first implementation, and Stable Growth first implementation. This does not change the P0 / P0.1 / P0.2 definitions above.

Accepted P1.1 behavior:

- supports only `strategy_type=ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing` for primary sample `000426`, `semiconductor_cycle` for primary sample `002371`, `advanced_manufacturing_growth` for primary sample `002050`, and `stable_growth` for primary sample `600406`;
...
- treats `output/research_intelligence_p1_*` and `output/research_questions_p1_*` as generated runtime artifacts that should not be committed;
...
- preserves `not_assessable` for missing AI Datacenter PUE / MW / cabinet / utilization / liquid-cooling revenue / customer-contract bridges, CXO order / backlog / utilization / regulatory bridges, Satellite transponder / bandwidth resources, utilization, contract duration, pricing, customer concentration, remaining life, launch / failure / insurance, and replacement-capex bridges, Low Altitude policy-pilot, airspace / route approval, flight-hour, flight-sortie, platform-dispatch, project-acceptance, customer-type, government / SOE collection-cycle, capex bridge, and safety / regulatory bridges, Resource realized-price / sales-volume / production / reserve / grade / capacity / hedging / cost-curve / capex-split / debt-maturity / environmental-safety bridges, Semiconductor cycle / localization / customer qualification / order / contract-liability / capex / inventory / export-control / R&D conversion bridges, Advanced Manufacturing robotics revenue / order / customer / mass-production / delivery / collection, customer concentration, receivable aging, inventory split, capex project mapping, and valuation evidence-sufficiency bridges, and Stable Growth multi-period demand durability, repeat-order / customer-retention, cash-conversion, collection-quality, capex-discipline, dividend / payout sustainability, ROE / ROIC, and valuation evidence-sufficiency bridges.

Accepted AI Datacenter pilot samples: `002837` and `300442`. Accepted Satellite primary sample: `601698`. Accepted Low Altitude primary sample: `000099`. Accepted Resource primary sample: `000426`. Accepted Semiconductor primary sample: `002371`. Accepted Advanced Manufacturing primary sample: `002050`. Accepted Stable Growth primary sample: `600406`. `resource_core` is design-only; `601899 / 603993` remain later Resource validation / boundary samples; `601689` remains a later Advanced Manufacturing validation / boundary sample; `002028` is Stable Growth validation / boundary only; `600276` is excluded from Stable Growth first-version support. Semiconductor first-version support is limited to `semiconductor_cycle + 002371` with equipment sub-chain as the first-version path. Semiconductor multi-sample observation is complete: `688012 / 688981 / 603501 / 300604` were upstream-refreshed validation samples, `300308 / 300476` were boundary / negative samples, and no validation sample was incorrectly mapped to the `002371` equipment first-version logic. Materials / fabless / foundry / OSAT are only `not_applicable` / `not_assessable` boundaries in the first version. Semiconductor guardrails: semiconductor cycle is not company performance; localization narrative is not revenue realization; R&D ratio is not a technology barrier or moat; customer introduction / certification / qualification is not batch revenue; contract liabilities are not backlog; capex is not capacity release, utilization, delivery, or revenue conversion; inventory movement is not demand judgment; export controls / sanctions are not company operating benefit or damage facts without company-level impact evidence. Advanced Manufacturing first-version support is limited to `advanced_manufacturing_growth + 002050`. Advanced Manufacturing multi-sample observation is complete, and the `601689` boundary data completion smoke is complete: `601689` was classified as `advanced_manufacturing_growth` but correctly remained `unsupported_pilot_strategy` / `not_assessable`, with fallback `company_transmission_path`, source-bucket independence, and the safety boundary preserved. The smoke did not apply the `002050` refrigeration / air-conditioning, automotive thermal-management, and robotics / actuator / emerging-business matrix, and did not write Tuopu large-customer or robotics narratives as realized revenue. Advanced Manufacturing guardrails: robotics theme is not revenue realization; strategic layout wording and `news` / `latest_news` are not transmission nodes; refrigeration / air-conditioning and automotive thermal-management financial data cannot proxy robotics realization; design-win / qualification / nomination is not batch revenue; contract liabilities are not backlog; capex is not capacity release, mass production, utilization, or revenue conversion; R&D ratio is not a technology barrier; receivable growth is not high-quality revenue; inventory movement is not demand judgment; valuation metrics are evidence-sufficiency context only and must not become target price, valuation-high/low, or trading judgment. Stable Growth first-version support is limited to `stable_growth + 600406`. Stable Growth acceptance recorded `missing / not_assessable = 19/33 = 57.58%`, which is expected and correct. Stable Growth guardrails: the `stable_growth` label is not operating-steadiness evidence; industry rigid demand, infrastructure, policy protection, and SOE / central-SOE attributes are not demand-durability evidence; revenue growth is not stable growth; single-period `revenue_yoy` plus same-direction operating cash flow is not multi-period stable-growth quality; contract liabilities are not backlog; receivable growth is not high-quality revenue; single-period OCF improvement is not long-term cash-flow stability; dividend / payout is not sustainable shareholder return without free cash flow, debt, capex, and earnings-quality support; single-period ROE is not long-term competitiveness; capex is not capacity release, utilization, future revenue, or growth conversion; valuation metrics are evidence-sufficiency context only and must not become valuation-high/low, target price, upside, or trading judgment. Data-side caveat: `601689` `news` / `latest_news` still reports `Invalid regular expression: invalid escape sequence: \u`; this is an upstream/news data issue and does not affect the P1.1 boundary smoke conclusion. Latest recorded validation after Stable Growth acceptance: `pytest` `484 passed`; regression suite `passed=47 failed=0 total=47`.
...
P1.1 Advanced Manufacturing status:

- first accepted implementation supports only `advanced_manufacturing_growth + 002050`;
- `601689` remains a later validation / boundary sample only;
- Advanced Manufacturing observation and `601689` boundary data completion smoke are complete;
- `601689` may be classified as `advanced_manufacturing_growth`, but P1.1 first-version behavior keeps it at `unsupported_pilot_strategy` / `not_assessable`;
- `601689` must not be force-fit into the `002050` refrigeration / air-conditioning, automotive thermal-management, and robotics / actuator / emerging-business matrix;
- Tuopu large-customer or robotics narratives must not be written as realized revenue without direct evidence;
- `601689` `news` / `latest_news` still has `Invalid regular expression: invalid escape sequence: \u`; this is upstream/news data caveat only and does not affect the P1.1 boundary smoke conclusion;
- refrigeration / air-conditioning core business, automotive thermal management, and robotics / actuator / emerging business must remain separated;
- robotics theme exposure is not revenue realization;
- strategic layout wording and `news` / `latest_news` are not valid transmission nodes;
- refrigeration / air-conditioning and automotive thermal-management data cannot proxy robotics realization;
- design-win / qualification / nomination is not batch revenue;
- contract liabilities are not backlog;
- capex is not capacity release, mass production, utilization, or revenue conversion;
- R&D ratio is not a technology barrier;
- receivable growth is not high-quality revenue;
- inventory movement is not demand judgment;
- valuation metrics are evidence-sufficiency context only, not target price, valuation-high/low, or trading judgment.
...
P1.1 Stable Growth status:

- first-version support is limited to `stable_growth + 600406`;
- `002028` is validation / boundary only and must not enter the first-version positive path;
- `600276` is excluded because the refreshed classifier result remains `unknown / insufficient_data`, and pharmaceutical / biotech pipeline risk can invalidate the ordinary Stable Growth premise;
- acceptance recorded `missing / not_assessable = 19/33 = 57.58%`, which is expected and correct;
- the `stable_growth` label, industry rigid demand, infrastructure, policy protection, and SOE / central-SOE attributes are not evidence for operating steadiness or demand durability;
- revenue growth, single-period `revenue_yoy` plus operating cash flow, contract liabilities, receivable growth, one-period OCF improvement, one-period ROE, aggregate capex, dividend / payout, and valuation metrics must remain within their evidence-sufficiency guardrails.
