...
next recommended stage is Minimal CNInfo / official disclosure parser design,
or A-share specific risk framework design. The next step is not more ad hoc
...
- Data Provider Phase 4 closeout / baseline freeze is complete. Tushare is usable for comparison but remains non-primary; AkShare / Tushare data must not be automatically merged; drift must not be automatically accepted. The third-smoke artifact root is timestamp `20260526T233804` under `output/provider_comparison/`; it can remain as a local, ignored, untracked review artifact. Ground Truth Benchmark, Auto Fact Candidate Generator, Candidate Report Review Protocol, Candidate Review Decisions Artifact, Research Report V1, User Invocation / Report Orchestration, User Invocation CLI / Command Wrapper, accepted artifact manifest / freshness design, and manifest locator runtime acceptance are documented; Research Report V1 implementation and baseline freeze are complete, the `600406`, `002371`, and `002050` Markdown profiles have passed cross-industry acceptance, the HTML renderer implementation is accepted, the three-sample HTML baseline is frozen, the three-sample offline orchestration runtime baseline is frozen, the single-stock offline CLI baseline is frozen, the CLI usage guide is recorded, and the manifest locator runtime baseline is frozen. The next recommended phase is Minimal CNInfo / official disclosure parser design or A-share specific risk framework design.
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  `docs/FUNDAMENTAL_ACCEPTED_MANIFEST_LOCATOR_RUNTIME_ACCEPTANCE_SUMMARY.md`.
  Next useful direction is Minimal CNInfo / official disclosure parser design
  or A-share specific risk framework design. HTML must
...
- Based on the accepted manifest / freshness design and manifest locator
  runtime acceptance, continue with Minimal CNInfo / official disclosure parser
  design or A-share specific risk framework design. Batch / Dashboard can be
...
A-share specific risk framework design. Do not continue single-stock CLI
