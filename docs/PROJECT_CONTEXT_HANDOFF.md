...
- `Data Provider Abstraction Phase 3 accepted`: `TushareClient` mocked abstraction and `TushareProvider` mocked mapping accepted; `ProviderRouter` optional modes remain `auto`, `akshare`, `tushare`, and `dual_compare`.
...
- For data-source migration, the next scoped step is Phase 4 dual-source comparison smoke design / local real-token acceptance planning. Do not read real tokens, connect MCP, call real Tushare, switch `provider=auto` primary behavior, or wire provider routing into the default production runner without a separate design / implementation / acceptance cycle. Phase 4 or real-token local acceptance may use a token only through a local environment variable or local MCP config; never paste tokens into prompts, code, docs, tests, logs, output, commits, or review comments.
