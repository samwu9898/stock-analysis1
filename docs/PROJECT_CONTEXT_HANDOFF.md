...
- For data-source migration, score / confidence explainability implementation and narrative hints are accepted, the Fundamental Ground Truth Benchmark fixture skeleton is present, the Auto Fact Candidate Generator design is documented, the `600406` V1.1 candidate report has been generated and passed usability review / audit, Candidate Report Review Protocol Design is accepted, and Candidate Review Decisions Artifact Design is documented. Next useful steps are review decisions artifact implementation, `600406` `candidate_review_decisions.json` generation / acceptance, then a separate promote-rule design, then controlled fixture promotion before any selected accepted facts enter the fixture, followed later by a standalone validator script, `fina_mainbz` `type=P/D/I` selected-period ratio derivation evaluation, and separate sidecar policy design. Do not run another real-token smoke unless later provider mapping or sidecar execution changes require it, do not request or read a token, do not connect MCP or read local MCP config, do not call Tushare or use the network, do not switch `provider=auto` primary behavior, do not auto-merge AkShare / Tushare data, do not auto-accept drift, and do not wire provider routing into the default production runner without a separate acceptance cycle.
