...
next recommended official-disclosure stage is Official Disclosure Business
Composition Table Parser Design. The
...
- `Fundamental Minimal CNInfo / Official Disclosure Parser`: design recorded in `docs/FUNDAMENTAL_MINIMAL_CNINFO_OFFICIAL_DISCLOSURE_PARSER_DESIGN.md`; local sample acceptance recorded in `docs/FUNDAMENTAL_OFFICIAL_DISCLOSURE_PARSER_LOCAL_SAMPLE_ACCEPTANCE_SUMMARY.md`; real local filing acceptance recorded in `docs/FUNDAMENTAL_OFFICIAL_DISCLOSURE_PARSER_REAL_LOCAL_FILING_ACCEPTANCE_SUMMARY.md`. The local-file implementation, Conservative Period Patch, local sample runtime review, and real local official filing sample runtime review are accepted. The real local accepted input is `output/official_disclosures/local_real_samples/600406_2025_semiannual_report_real.txt`, and the ignored runtime artifact is `output/official_disclosures/20260528T125521Z/600406/official_disclosure_facts.json`. The validated loop is local real filing text -> reader -> conservative extractors -> business-composition section detection with caveat -> builder -> writer -> reader -> validator. The result recorded `document_type=semiannual_report`, `report_period=2025H1`, `disclosure_date=2025-08-28`, main-business L1 candidate from `source_section=主营业务`, `source_documents=1`, `extracted_facts=1`, source location for L1 facts, and `not_for_trading_advice=true`. Business composition regions were detected, but revenue, cost, gross margin, revenue ratio, YoY, and segment values were not extracted because copied PDF TXT table structure is unreliable. This is not live CNInfo, not original parsed PDF, not PDF table parsing, not a fixture, not a regression expected file, not an accepted manifest update, not candidate generator integration, and not a Research Report V1 integration. It does not call CNInfo, call providers, read tokens, use the network, connect MCP, write fixtures, update scoring / readiness, change P1.1, change regression expected files, or provide trading advice. Next recommended stage is Official Disclosure Business Composition Table Parser Design.
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  and real local filing acceptance are recorded. Next useful official-disclosure
  direction is Official Disclosure Business Composition Table Parser Design.
  HTML must
...
- Based on the accepted manifest / freshness design, manifest locator runtime
  acceptance, official disclosure parser local sample acceptance, and real
  local filing acceptance, continue with Official Disclosure Business
  Composition Table Parser Design. Batch / Dashboard can be
...
acceptance, official disclosure parser local sample acceptance, and real local
filing acceptance are recorded, and the next official-disclosure work should
enter Official Disclosure Business Composition Table Parser Design.
...
Next recommended stage: Official Disclosure Business Composition Table Parser
Design. Live CNInfo fetch, MCP, provider calls, token work, validator work,
fixture promotion, candidate generator integration, Research Report V1
integration, scoring changes, P1.1 changes, manifest updates, report rewrites,
batch, and Dashboard remain separate later stages.
