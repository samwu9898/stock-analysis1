...
- For data-source migration, score / confidence explainability implementation and narrative hints are accepted, the Fundamental Ground Truth Benchmark fixture skeleton is present, and the Auto Fact Candidate Generator design is documented. Next useful steps are offline candidate generator implementation from existing provider-comparison artifacts, a `600406` candidate report, candidate review / promotion of selected accepted facts into the fixture, a standalone validator script, later `fina_mainbz` `type=P/D/I` selected-period ratio derivation evaluation, and separate sidecar policy design. Do not run another real-token smoke unless later provider mapping or sidecar execution changes require it, do not request or read a token, do not connect MCP or read local MCP config, do not call Tushare or use the network, do not switch `provider=auto` primary behavior, do not auto-merge AkShare / Tushare data, do not auto-accept drift, and do not wire provider routing into the default production runner without a separate acceptance cycle.
