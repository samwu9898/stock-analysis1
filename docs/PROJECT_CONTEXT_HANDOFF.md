...
- `Data Provider Abstraction`: Phase 1 skeleton, Phase 2 `AkShareProvider` adapter, and Phase 3 `TushareProvider` mocked MVP have been accepted. Phase 4 dual-source comparison smoke design / local real-token acceptance planning is documented in `docs/DATA_PROVIDER_PHASE4_DUAL_SOURCE_COMPARISON_DESIGN.md`. Phase 3 added a mocked-only `TushareClient` abstraction and `TushareProvider` canonical raw mapping; it did not connect real Tushare, read token / env values, read local MCP config, call MCP, call the network, switch provider primary behavior, or wire into the default `real_stock_runner` production path. `AkShareProvider` remains a thin wrapper around `RealDataConnector`.
...
- `Data Provider Abstraction Phase 3 accepted / Phase 4 design documented`: `TushareClient` mocked abstraction and `TushareProvider` mocked mapping accepted; `ProviderRouter` optional modes remain `auto`, `akshare`, `tushare`, and `dual_compare`. Phase 4 implementation, if started, must keep comparison artifacts isolated under `output/provider_comparison/<timestamp>/<code>/`, keep default production output unchanged, keep real-token smoke behind an explicit local-only flag, and avoid any automatic primary-provider switch.
...
- Data Provider Phase 4 design documentation is added in `docs/DATA_PROVIDER_PHASE4_DUAL_SOURCE_COMPARISON_DESIGN.md`. Phase 4 is a comparison-only / local-acceptance-planning stage: `AkShareProvider -> raw dict -> FundamentalSkillPipeline.analyze_from_dict -> EvidencePackBuilder.build` versus `TushareProvider -> raw dict -> FundamentalSkillPipeline.analyze_from_dict -> EvidencePackBuilder.build`; artifacts must stay under `output/provider_comparison/<timestamp>/<code>/`; P1.1 comparison is off by default and requires an explicit include-P1 option; real-token smoke is off by default and requires an explicit local-only flag; no default runner, provider primary, regression expected, HTML / Dashboard, or P1.1 default-chain behavior should change.
...
- For data-source migration, the next scoped step is Phase 4 implementation after accepting `docs/DATA_PROVIDER_PHASE4_DUAL_SOURCE_COMPARISON_DESIGN.md`. Minimum scope: isolated `compare_providers` runner, diff classifier, artifact writer, dry-run mode, token leak scanner, and provider-separated pipeline / evidence builder. Do not read real tokens, connect MCP, call real Tushare, switch `provider=auto` primary behavior, or wire provider routing into the default production runner without a separate implementation / acceptance cycle. Real-token local acceptance may use a token only through a local environment variable or local MCP config; never paste tokens into prompts, code, docs, tests, logs, output, commits, or review comments.
