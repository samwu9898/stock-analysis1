...
Period Patch, and local sample runtime review are accepted; the documentation
...
next recommended stage is either `official_disclosure_facts -> candidate
generator integration design`, real local downloaded official filing sample
review, or A-share specific risk framework design. The
...
primary, live CNInfo fetch, live provider report, MCP, Tushare token work, or
another smoke.
...
implementation and local sample runtime review are accepted, but it is not
wired into the accepted CLI baseline, is not a live CNInfo fetch path, and does
not yet make L1 official evidence available to Research Report V1.
...
- `Fundamental Minimal CNInfo / Official Disclosure Parser`: design recorded in `docs/FUNDAMENTAL_MINIMAL_CNINFO_OFFICIAL_DISCLOSURE_PARSER_DESIGN.md`; local sample acceptance recorded in `docs/FUNDAMENTAL_OFFICIAL_DISCLOSURE_PARSER_LOCAL_SAMPLE_ACCEPTANCE_SUMMARY.md`. The local-file implementation, Conservative Period Patch, and local sample runtime review are accepted. The accepted sample is `output/official_disclosures/local_samples/600406_annual_report_sample.txt`, and the ignored runtime artifact is `output/official_disclosures/20260528_194020/600406/official_disclosure_facts.json`. The validated loop is local sample text -> reader -> conservative extractors -> builder -> writer -> reader -> validator. The result recorded `document_type=annual_report`, `report_period=2025A`, `disclosure_date=2026-04-30`, main-business candidate from `主营业务`, `source_documents=1`, `extracted_facts=4`, source locations for all L1 facts, and `not_for_trading_advice=true`. This is not live CNInfo, not a complete real announcement parse, not PDF or complex-table parsing, not a fixture, not a regression expected file, not an accepted manifest update, and not a Research Report V1 integration. It does not call CNInfo, call providers, read tokens, use the network, connect MCP, write fixtures, update scoring / readiness, change P1.1, change regression expected files, or provide trading advice. Next recommended stage is `official_disclosure_facts -> candidate generator integration design`, or real local downloaded official filing sample review from a user-provided local text file.
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  `docs/FUNDAMENTAL_ACCEPTED_MANIFEST_LOCATOR_RUNTIME_ACCEPTANCE_SUMMARY.md`.
  Minimal CNInfo / official disclosure parser design and local sample
  acceptance are recorded. Next useful direction is
  `official_disclosure_facts -> candidate generator integration design`, real
  local downloaded official filing sample review, or A-share specific risk
  framework design. HTML must
...
- Based on the accepted manifest / freshness design, manifest locator runtime
  acceptance, and official disclosure parser local sample acceptance, continue
  with `official_disclosure_facts -> candidate generator integration design`,
  real local downloaded official filing sample review, or A-share specific
  risk framework design. Batch / Dashboard can be
...
Minimal CNInfo / official disclosure parser design and local sample runtime
acceptance are now recorded. Next recommended stage:
`official_disclosure_facts -> candidate generator integration design`, real
local downloaded official filing sample review, or A-share specific risk
framework design. Do not continue single-stock CLI
...
acceptance, and official disclosure parser local sample acceptance are
recorded, and the next work should enter `official_disclosure_facts ->
candidate generator integration design`, real local downloaded official filing
sample review, or A-share specific risk framework design.
...
`docs/FUNDAMENTAL_OFFICIAL_DISCLOSURE_PARSER_LOCAL_SAMPLE_ACCEPTANCE_SUMMARY.md`
as the current local sample runtime acceptance closeout. The parser local-file
implementation is accepted, but it does not alter the accepted offline CLI
baseline.
...
integration design`, or real local downloaded official filing sample review
from a user-provided local text file. Live CNInfo fetch, MCP, provider calls,
token work, validator work, fixture promotion, scoring changes, P1.1 changes,
