...
next recommended stage is parser schema / local-file reader implementation for
the official disclosure parser, or A-share specific risk framework design. The
...
primary, live provider report, MCP, Tushare token work, or another smoke.
...
The Minimal CNInfo / official disclosure parser design is documentation-only
infrastructure for future official-evidence candidates. It is not wired into
the accepted CLI baseline and is not a live CNInfo fetch path.
...
- `Fundamental Minimal CNInfo / Official Disclosure Parser Design`: documentation-only design recorded in `docs/FUNDAMENTAL_MINIMAL_CNINFO_OFFICIAL_DISCLOSURE_PARSER_DESIGN.md`. It defines future local-file-first official disclosure parsing for annual / semiannual / quarterly reports, official `main_business` text, business composition, earnings preannouncement / flash / guidance placeholders, and A-share risk triggers. Future output is designed as `output/official_disclosures/<timestamp>/<code>/official_disclosure_facts.json`, with `L1_official_disclosure` candidates carrying source document, source section, page / anchor, parser confidence, human-review flags, and caveats. It does not implement code, call CNInfo, call providers, read tokens, use the network, connect MCP, write fixtures, update the accepted manifest, change Research Report V1, change scoring / readiness, change P1.1, change regression expected files, or provide trading advice. Next implementation step, if opened separately, is parser schema / local-file reader implementation.
...
- Data Provider Phase 4 closeout / baseline freeze is complete. Tushare is usable for comparison but remains non-primary; AkShare / Tushare data must not be automatically merged; drift must not be automatically accepted. The third-smoke artifact root is timestamp `20260526T233804` under `output/provider_comparison/`; it can remain as a local, ignored, untracked review artifact. Ground Truth Benchmark, Auto Fact Candidate Generator, Candidate Report Review Protocol, Candidate Review Decisions Artifact, Research Report V1, User Invocation / Report Orchestration, User Invocation CLI / Command Wrapper, accepted artifact manifest / freshness design, manifest locator runtime acceptance, and Minimal CNInfo / official disclosure parser design are documented; Research Report V1 implementation and baseline freeze are complete, the `600406`, `002371`, and `002050` Markdown profiles have passed cross-industry acceptance, the HTML renderer implementation is accepted, the three-sample HTML baseline is frozen, the three-sample offline orchestration runtime baseline is frozen, the single-stock offline CLI baseline is frozen, the CLI usage guide is recorded, and the manifest locator runtime baseline is frozen. The next recommended phase is parser schema / local-file reader implementation or A-share specific risk framework design.
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  `docs/FUNDAMENTAL_ACCEPTED_MANIFEST_LOCATOR_RUNTIME_ACCEPTANCE_SUMMARY.md`.
  Minimal CNInfo / official disclosure parser design is recorded. Next useful
  direction is parser schema / local-file reader implementation or A-share
  specific risk framework design. HTML must
...
- Based on the accepted manifest / freshness design, manifest locator runtime
  acceptance, and Minimal CNInfo / official disclosure parser design, continue
  with parser schema / local-file reader implementation or A-share specific
...
Minimal CNInfo / official disclosure parser design is now recorded. Next
recommended stage: parser schema / local-file reader implementation for the
official disclosure parser, or A-share specific risk framework design. Do not
continue single-stock CLI
...
frozen. Accepted artifact manifest / freshness design and manifest locator
runtime acceptance are recorded, and the next work should enter Minimal CNInfo
/ official disclosure parser design or A-share specific risk framework design.
...
current design source for future official disclosure parsing. It is a
documentation-only stage and does not alter the accepted offline CLI baseline.
...
Next recommended implementation stage: parser schema / local-file reader
implementation, then one local downloaded filing sample for official
`main_business` and `business_composition` extraction. Live CNInfo fetch, MCP,
provider calls, token work, fixture promotion, scoring changes, P1.1 changes,
