...
`docs/FUNDAMENTAL_ACCEPTED_ARTIFACT_MANIFEST_FRESHNESS_DESIGN.md`. The next
recommended stage is manifest schema / writer / reader implementation and
locator hardening. The next step is not more ad hoc single-target HTML
generation, more single-stock CLI runtime generation, CLI usage documentation,
batch / Dashboard, promote rules, validator, fixture promotion, Tushare
primary, live provider report, official parser / CNInfo, MCP, Tushare token
work, or another smoke.
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  `docs/FUNDAMENTAL_ACCEPTED_ARTIFACT_MANIFEST_FRESHNESS_DESIGN.md`. Next
  useful direction is manifest schema / writer / reader implementation and
  locator hardening. HTML must
...
- Based on the accepted manifest / freshness design, continue with manifest
  schema / writer / reader implementation and locator hardening. Do not
  continue with batch / Dashboard, do not generate more ad hoc single-stock
  Research Report V1 HTML artifacts, do not continue single-stock CLI runtime
  generation, and do not redo CLI usage documentation unless a new sample,
  regression check, or usage-surface change explicitly requests that scope.
...
Next recommended stage: manifest schema / writer / reader implementation,
followed by locator hardening so orchestration and CLI read the accepted
manifest before timestamp-latest fallback. Do not continue single-stock CLI
...
Batch / Dashboard, promote rules, validator, fixture promotion, live provider
report, official parser / CNInfo, MCP, Tushare token work, and Tushare primary
remain later work.
...
- Future locator behavior should read
...
2. Implement manifest schema / writer / reader.
3. Harden orchestration / CLI locator behavior to prefer the manifest.
4. Generate ignored runtime manifests for `600406`, `002371`, and `002050`.
5. Run manifest locator runtime acceptance.
...
frozen. Accepted artifact manifest / freshness design is recorded, and the next
work should enter manifest schema / writer / reader implementation plus locator
hardening.
...
## 17. New Codex Conversation Recovery Prompt
