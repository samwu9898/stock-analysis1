...
`docs/FUNDAMENTAL_SKILL_OFFLINE_ORCHESTRATION_ACCEPTANCE_SUMMARY.md`, and the
CLI / command wrapper design is recorded in
`docs/FUNDAMENTAL_SKILL_USER_INVOCATION_CLI_DESIGN.md`. The
...
invocation CLI / command wrapper implementation; Dashboard / batch should
follow only after the single-stock CLI implementation is accepted. The next
step is not more ad hoc single-target HTML generation, promote rules,
validator, fixture promotion, Tushare primary, live provider report, official
parser / CNInfo, MCP, Tushare token work, or another smoke.
...
- `Fundamental Skill User Invocation / Report Orchestration`:
  ...
  `docs/FUNDAMENTAL_SKILL_USER_INVOCATION_CLI_DESIGN.md`; implementation is the
...
- Current Research Report V1 presentation and invocation step: the HTML
  ...
  `docs/FUNDAMENTAL_SKILL_OFFLINE_ORCHESTRATION_ACCEPTANCE_SUMMARY.md`, and the
  CLI / command wrapper design is recorded in
  `docs/FUNDAMENTAL_SKILL_USER_INVOCATION_CLI_DESIGN.md`. Next useful direction
  is user invocation CLI / command wrapper implementation. Dashboard / batch
  should follow only after the single-stock CLI implementation is accepted. HTML must
...
- Based on user feedback, continue with user invocation CLI / command wrapper
  implementation. Do not continue generating more ad hoc single-stock
  Research Report V1 HTML artifacts, and do not move to Dashboard / batch
  before the single-stock CLI implementation is accepted unless a new stage explicitly
  requests that scope.
...
Next recommended stage: user invocation CLI / command wrapper implementation,
for example
`python -m src.fundamental_skill.research_report.generate_report --code 600406 --format html --data-mode offline_local_artifacts`.
Dashboard / batch should follow only after the single-stock CLI implementation
is accepted.
...
command-wrapper design. The next work should implement a single user-facing CLI
/ command wrapper that hides `fact_candidates`, `candidate_review_decisions`,
JSON, Markdown, and HTML internals from the user.
