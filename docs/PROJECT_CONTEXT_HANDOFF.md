...
- `Fundamental Research Report V1 Design`: documentation-only design for returning from the data-quality audit chain to professional fundamental research output. It defines a `research_report_v1` structure covering executive summary, data quality, macro context, industry context, company fundamentals, opportunities, risks, evidence gaps, rebuttal conditions, follow-up variables, evidence labels, source artifact references, and `600406` grid-equipment / stable-growth expected behavior. It consumes existing artifacts conceptually but does not call providers, read tokens, connect MCP, write output, promote fixtures, alter scoring / readiness, alter P1.1, or provide trading advice.
