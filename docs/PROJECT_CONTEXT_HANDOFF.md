...
- `Data Provider Abstraction`: Phase 1 skeleton and Phase 2 `AkShareProvider` adapter have been accepted. `AkShareProvider` is a thin wrapper around `RealDataConnector`; it preserves the canonical raw JSON contract and is not wired into the default `real_stock_runner` production path.
...
- `Data Provider Abstraction Phase 2`: `AkShareProvider` adapter accepted; `ProviderRouter` optional modes are `auto`, `akshare`, `tushare`, and `dual_compare`.
...
- Tushare migration remains pre-MVP. Phase 3 is the first `TushareProvider` MVP phase and should use mocks / fake tokens until a separate explicit real-token acceptance step is designed. Real tokens must never be written to code, docs, tests, logs, output, commits, or review comments; docs may only use `<TUSHARE_TOKEN>` as a placeholder, and MCP URL text from local config must not enter the repository.
...
- Keep `docs/DATA_PROVIDER_ABSTRACTION_TUSHARE_MIGRATION_DESIGN.md` frozen as the Phase 2 accepted baseline unless a new provider-migration phase explicitly updates it.
- For data-source migration, the next scoped step is Phase 3 `TushareProvider` MVP with mockable client boundaries. Do not read real tokens, connect MCP, call real Tushare, or wire provider routing into the default production runner without a separate design / implementation / acceptance cycle.
