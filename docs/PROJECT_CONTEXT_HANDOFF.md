...
- `Data Provider Abstraction`: Phase 1 skeleton and Phase 2 `AkShareProvider` adapter have been accepted. Phase 3 `TushareProvider` MVP design audit is synchronized in `docs/DATA_PROVIDER_ABSTRACTION_TUSHARE_MIGRATION_DESIGN.md` as a mocked-only implementation basis. `AkShareProvider` is a thin wrapper around `RealDataConnector`; it preserves the canonical raw JSON contract and is not wired into the default `real_stock_runner` production path.
...
- `Data Provider Abstraction Phase 3 design`: Phase 3 `TushareProvider` MVP design synchronized; next implementation should use `TushareClient` abstraction, mocked transport, mocked response mapping, token safety, no-token fail-closed behavior, and mock tests only. `ProviderRouter` optional modes remain `auto`, `akshare`, `tushare`, and `dual_compare`.
...
- Tushare migration Phase 3 design is synchronized. Phase 3 is a mocked-only `TushareProvider` MVP basis: `TushareClient` abstraction, mocked transport, mocked response mapping for `basic_info`, `financial_indicator`, `valuation`, `business_composition`, explicit missing / fallback `news`, `fetch_status` / `errors` / `source_trace`, token safety, no-token fail-closed behavior, and mock tests only. Phase 3 must not do real token smoke, MCP integration, real network calls in tests, `provider=auto` primary switch, dual-source comparison, news replacement, `commodity_prices` replacement, industry-specific operating indicators, minute / realtime technical data, classifier / scoring / readiness changes, `evidence_pack` schema changes, HTML / P1.1 changes, or regression expected changes. Real tokens must never be written to code, docs, tests, logs, output, commits, or review comments; docs may only use `<TUSHARE_TOKEN>` as a placeholder, and MCP URL text from local config must not enter the repository.
...
- For data-source migration, the next scoped step is Phase 3 mocked implementation: `TushareClient` abstraction with mocked transport, `TushareProvider` mocked mapping into canonical raw blocks, ProviderRouter injection tests, no-token / masking tests, and optional local-only smoke design that is not executed by default. Do not read real tokens, connect MCP, call real Tushare, switch `provider=auto` primary behavior, or wire provider routing into the default production runner without a separate design / implementation / acceptance cycle.
