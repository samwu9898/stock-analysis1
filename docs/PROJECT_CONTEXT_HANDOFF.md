...
- `Data Provider Abstraction`: Phase 1 skeleton, Phase 2 `AkShareProvider` adapter, Phase 3 `TushareProvider` mocked MVP, Phase 4 dual-source comparison dry-run tooling, Phase 4 local real-token smoke gate documentation, and Phase 4 real-token smoke gate safety skeleton have been accepted. Phase 4 is documented in `docs/DATA_PROVIDER_PHASE4_DUAL_SOURCE_COMPARISON_DESIGN.md`, `docs/DATA_PROVIDER_PHASE4_LOCAL_REAL_TOKEN_SMOKE_GATE.md`, and `docs/DATA_PROVIDER_PHASE4_REAL_TOKEN_SMOKE_IMPLEMENTATION_DESIGN.md`; it added the isolated `compare_providers` runner plus `comparison_artifacts`, `diff_classifier`, `token_leak_scanner`, `real_token_smoke_gate`, and `tushare_sdk_transport`. The accepted safety skeleton adds precheck / runtime / postcheck / cleanup guardrails, SDK fake-object / factory injection, `--real-token-smoke --provider-transport sdk` interlock, reserved fail-closed `http` / `mcp-local`, `--token` rejection, strengthened token scanning, and `strategy_type_drift`. Phase 4 still did not execute real smoke, connect real Tushare, read a real `TUSHARE_TOKEN`, read local MCP config, call MCP, call the network, switch provider primary behavior, wire provider routing into the default `real_stock_runner` production path, change deterministic pipeline behavior, change `evidence_pack`, change Research Intelligence P1.1, change HTML / Dashboard, change classifier / scoring / readiness, generate `output/provider_comparison`, or change regression expected outputs. `AkShareProvider` remains a thin wrapper around `RealDataConnector`.
...
- `Data Provider Abstraction Phase 4 dry-run + real-token smoke gate safety skeleton accepted`: `TushareClient` mocked abstraction and `TushareProvider` mocked mapping accepted; `ProviderRouter` optional modes remain `auto`, `akshare`, `tushare`, and `dual_compare`; `compare_providers`, `comparison_artifacts`, `diff_classifier`, `token_leak_scanner`, `real_token_smoke_gate`, and `tushare_sdk_transport` are accepted isolated tooling. Phase 4 artifact writes, when explicitly enabled, are isolated under `output/provider_comparison/<timestamp>/<code>/`; default dry-run does not generate `output/provider_comparison`, does not write production output, does not run HTML, and does not run Research Intelligence P1.1. `--include-p1` is off by default. The real-token smoke path requires explicit `--real-token-smoke --provider-transport sdk`, fails closed without a token before SDK calls, rejects `--token`, and keeps `http` / `mcp-local` reserved fail-closed. No real smoke has been executed. There is still no automatic primary-provider switch.
...
- Provider migration roadmap snapshot: Phase 0 completed; Phase 1 accepted; Phase 2 accepted; Phase 3 accepted; Phase 4 dry-run accepted; Phase 4 local real-token smoke gate documentation completed; Phase 4 real-token smoke gate safety skeleton implemented and accepted; next is local real-token smoke execution acceptance review / external audit gate.
- Data Provider Phase 4 dual-source comparison dry-run tooling is implemented and accepted. Phase 4 remains comparison-only / local-acceptance-gated: `AkShareProvider -> raw dict -> FundamentalSkillPipeline.analyze_from_dict -> EvidencePackBuilder.build` versus `TushareProvider -> raw dict -> FundamentalSkillPipeline.analyze_from_dict -> EvidencePackBuilder.build`. Added modules are `compare_providers`, `comparison_artifacts`, `diff_classifier`, `token_leak_scanner`, `real_token_smoke_gate`, and `tushare_sdk_transport`. Default dry-run does not generate `output/provider_comparison`, does not write production output, does not run HTML, and does not run P1.1. Artifact writes, when explicitly enabled, are allowed only under `output/provider_comparison/<timestamp>/<code>/`; forbidden writes include `output/raw_<code>.json`, `output/fundamental_<code>.json`, `output/evidence_pack_<code>.json`, and `output/reports`. P1.1 comparison is off by default and requires `--include-p1`; real-token smoke requires explicit `--real-token-smoke --provider-transport sdk`, rejects `--token`, fails closed with no token before SDK calls, and keeps `http` / `mcp-local` reserved fail-closed. The gate scans repo tracked files, staged diff, docs / tests / source, target output, payloads, and diff reports; it baselines `output/reports` and default output path sets plus SHA-256 hashes; cleanup is limited to a strict timestamp directory under `output/provider_comparison`. The diff classifier marks `strategy_type_drift`, classification / confidence / score / P1 drift as `review_required`, treats `token_or_secret_risk` as blocker, and does not automatically accept drift. The token scanner supports exact references, `dict` / `list` / `str`, dict keys / values, URL query parameters, keyword-proximity detection, `token=`, `api_key=`, `Bearer`, MCP URL / `mcp?token=`, and realistic 32+ token-like values, and emits only location plus `<masked>`. Phase 4 did not execute real smoke, read a real token, connect real Tushare, read MCP config, call MCP, use network, switch primary provider, change default runner, change regression expected files, generate `output/provider_comparison`, modify default output, or modify `output/reports`. Latest Phase 4 verification: targeted tests `42 passed, 1 skipped`; full `pytest` `589 passed, 1 skipped`; regression suite `passed=47 failed=0 total=47`.
...
- For data-source migration, Phase 4 dry-run and real-token smoke gate safety skeleton are accepted. The next scoped step is local real-token smoke execution acceptance review / external audit gate, not direct execution of a real-token smoke. Do not read real tokens, connect MCP, call real Tushare, switch `provider=auto` primary behavior, or wire provider routing into the default production runner without a separate execution acceptance cycle. Real-token local execution may use a token only through local `TUSHARE_TOKEN` or local MCP config; never paste tokens into prompts, code, docs, tests, logs, output, commits, or review comments.
