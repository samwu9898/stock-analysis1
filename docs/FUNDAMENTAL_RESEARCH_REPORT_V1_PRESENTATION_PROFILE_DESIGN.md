...
Status: design-only. This document does not implement code, modify tests,
modify fixtures, modify pipeline behavior, modify scoring / readiness, modify
Research Intelligence P1.1, modify regression expected files, generate output,
run smoke tests, read `TUSHARE_TOKEN`, use the network, call Tushare or
AkShare, connect MCP, or provide buy / sell advice, target prices, position
...
600406 refined Markdown 已通过产品可读性初验，但 renderer 已围绕 600406 /
电网设备打磨。下一步支持 002371 / 002050 之前，必须先建立 profile layer，
避免 600406 的电网设备语言污染半导体设备和热管理 /先进制造报告。
...
## 8. 未来 implementation 建议

建议后续实现以小而可审计的 registry 为主：

- 新增 `ResearchReportPresentationProfile` 或 dict-based profile registry。
- 新增 `_select_presentation_profile(report)`。
...
实现顺序应先 registry 和 selection，再改 renderer。不要直接把 600406 renderer
套到 002371 / 002050。
...
5. 做跨行业可读性验收。
6. 再考虑 HTML presentation layer。

完成 profile registry implementation 之前，不建议生成 002371 / 002050 的正式
Markdown 报告；可以先用 fake reports 做 selection、term isolation 和 evidence
boundary 测试。
