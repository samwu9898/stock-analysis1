...
Research Intelligence P1.1 is the accepted independent driver-factor matrix artifact in the AI analyst layer. It reads the existing evidence pack, may read the P0 pack, and writes an independent driver-factor matrix plus driver research questions:

- `output/research_intelligence_p1_<code>.json`
- `output/research_questions_p1_<code>.json`
- `output/research_questions_p1_<code>.md`

These `output/research_intelligence_p1_*` and `output/research_questions_p1_*` files are generated runtime artifacts and should not be committed.

P1.1 currently supports `ai_datacenter_infrastructure`, `life_science_cxo_services`, `satellite_communication_infrastructure`, `low_altitude_economy_infrastructure`, `resource_swing`, `semiconductor_cycle`, `advanced_manufacturing_growth`, and `stable_growth`. The Satellite, Low Altitude, Resource, Semiconductor, Advanced Manufacturing, and Stable Growth expansions are still AI analyst-layer independent artifacts: they do not change deterministic pipeline behavior, classifier output, connector mappings, scoring, readiness, result assembly, `status`, `confidence`, `score`, `strategy_type`, or `sub_type`. They do not output trading advice, target prices, position sizing, technical analysis, or K-line content, and they do not connect to the HTML Report / Dashboard main chain. Low Altitude P1.1 remains evidence-pack-only and must keep missing flight hours, flight sorties, airspace / route approval, project acceptance, customer type, government / SOE collection cycle, contract liabilities, and capex bridge items evidence-gated. Resource P1.1 first implementation supports only `resource_swing + 000426`; `resource_core` is design-only, `601899 / 603993` remain later validation / boundary samples, and commodity price, commodity cycle, reserves, production volume, capex, inventory, and hedging must remain evidence-gated rather than converted into company facts. Semiconductor P1.1 first implementation supports only `semiconductor_cycle + 002371`; equipment is the first-version sub-chain path. Semiconductor multi-sample observation is complete: `688012 / 688981 / 603501 / 300604` were validation samples, `300308 / 300476` were boundary / negative samples, and no validation sample was force-fit into the `002371` equipment first-version order logic. Materials / fabless / foundry / OSAT are not fully implemented beyond `not_applicable` / `not_assessable` boundary handling. Advanced Manufacturing P1.1 first implementation supports only `advanced_manufacturing_growth + 002050`; the Advanced Manufacturing multi-sample observation is complete, and the `601689` boundary data completion smoke is complete. `601689` was classified as `advanced_manufacturing_growth` but correctly remained `unsupported_pilot_strategy` / `not_assessable`; it is a later validation / boundary sample and is not first-version support. The smoke preserved fallback `company_transmission_path`, source-bucket independence, and the safety boundary, and did not apply the `002050` three-business-layer matrix or write Tuopu large-customer / robotics narratives as realized revenue. Stable Growth P1.1 first implementation supports only `stable_growth + 600406`; `002028` is validation / boundary only; `600276` is excluded because the refreshed classifier result remains `unknown / insufficient_data` and pharmaceutical / biotech pipeline risk can invalidate the ordinary Stable Growth premise. Stable Growth acceptance recorded `missing / not_assessable = 19/33 = 57.58%`, which is expected and correct. Data-side caveat: `601689` `news` / `latest_news` still reports `Invalid regular expression: invalid escape sequence: \u`; this is an upstream/news data issue and does not affect the P1.1 boundary smoke conclusion. Latest recorded validation after Stable Growth acceptance: `pytest` `484 passed`; regression suite `passed=47 failed=0 total=47`.
...
P1.1 Advanced Manufacturing 第一版已验收，范围只支持 `advanced_manufacturing_growth + 002050`。`601689` 仅作为后续 validation / boundary sample，不进入第一版正式支持。P1.1 artifact 必须隔离制冷空调核心、汽车热管理、机器人/执行器/新兴业务三层；机器人层缺少独立非零收入、订单、客户、量产、回款证据时，默认 fallback + `not_assessable`。

Advanced Manufacturing observation is complete, including the `601689` boundary data completion smoke: `601689` may be classified as `advanced_manufacturing_growth`, but P1.1 first-version behavior must keep it at `unsupported_pilot_strategy` / `not_assessable`, must not apply the `002050` three-layer matrix, and must not write Tuopu large-customer or robotics narratives as realized revenue. The remaining `news` / `latest_news` `Invalid regular expression: invalid escape sequence: \u` caveat is upstream/news data only.
